App = {
  ...
  pageCallback: async function (index, jq) {
    ...
    for (var i = start; i < end; i++) {
      ...
      content +=
        ...
        '<img id="cover" class="img-cover" src="' +
        result[6] +
        '" alt="资讯封面"/>' +
        '<figcaption id="nameWriter" class="img-caption">' +
        result[1] +
        "</figcaption>" +
        "</div>" +
        "</a>" +
  ...
  _getPublishedBooks: function () {
    return new Promise(function (resolve, reject) {
      book.deployed().then(function (bookInstance) {
        bookInstance.getPublishedBooks
          .call()
          .then(function (result) {
            resolve(result);
          })
