...
App = {
  ...
  getBooks: async function () {
    ...
    $("#owner").html(result[0]);
    $("#intro").html(result[4]);
    $("#date").html(fmtBookDate(result[8].toString()));
    $("#cover").attr("src", result[5]);
    ...
    buttonContent +=
      ...
      '<span class="name" id="nameWriter">' +
      result[1] +
      "</span>" +
      '<span class="btn-primary" id="style">' +
      result[2] +
      "</span>" +
      '<p class="normal">评分：<samp id="score">' +
      result[9].toString() +
      "</samp></p>" +
      '<p class="normal">所属年代：<samp id="publisherPublishAge">' +
      result[3] +
      "</samp></p>" +
      '<p class="normal">资料内容：<samp id="pdf">' +
      '<a href="' +
      result[11] +
      '">点击这里跳转</a>' +
      "</samp></p>" +
      '<p class="normal">资料来源：<samp id="pages">' +
      '<a href="' +
      result[7] +
      '">点击这里跳转</a>' +
      "</samp></p>" +
      '<p class="normal">在线状态：<samp id="status">' +
      result[6] +
      "</samp></p>" +
      '<p class="normal">借阅次数：<samp id="borrowNums">' +
      borrowNum +
      "</samp></p>" +
      '<p class="buy" id="bottonCentent">' +
      '<button id="comment"  style="background-color: red" onclick="App.set(' +
      gid +
      ')">立即评价</button>' +
    ...
    for (var i = 0; i < clen; i++) {
      ...
      if (bookInfo[0] == result[0]) userPic = "images/owner.png";
      else userPic = "images/buyer.png";
      content +=
        '<div class="row">' +
        '<div class="col-sm-1">' +
        "<img src=" +
        ...
        '<div class="col-sm-11">' +
        "<p>" +
        fmtBookDate(result[1].toString()) +
        "</p>" +
        '<p name="star" data-score="' +
        result[2] +
        '"></p>' +
        "<p>" +
        result[3] +
  ...
  bookevaluate: async function () {
    ...
    book.deployed().then(function (bookInstance) {
      bookInstance
        .evaluate(evaluateId, evaluateScore, content, {
